...
// ---- 詳細サマリー（一覧の行を展開して表示） ----------------------------------------------
function DetailBlock({ t, todayIso }: { t: ClientWbsTask; todayIso: string | null }) {
  return (
    <div className="space-y-4 text-base leading-relaxed text-ink sm:text-sm">
      <section className="rounded-lg border border-line bg-paper px-4 py-3">
        <SectionHeading>概要（目的・実施・結果）</SectionHeading>
      ...
      <dl className="grid grid-cols-[6rem_1fr] gap-x-3 gap-y-1.5 text-sm">
        ...
        <dd className="flex flex-wrap items-center gap-2"><Period t={t} />{isOverdue(t, todayIso) && <OverdueBadge />}</dd>
      ...
      <div className="overflow-x-auto rounded-xl border border-line bg-white">
        <div className="min-w-[720px]">
          <div className="grid" style={{ gridTemplateColumns: "minmax(240px,1.2fr) 3fr" }}>
            <div className="border-b border-line px-4 py-2 text-xs text-ink-soft">施策（クリックで一覧の詳細へ）</div>
...
export default function ClientWbsBoard({ data }: { data: ClientWbsData }) {
  ...
  const months = useMemo(() => {
    const yms = data.tasks
      .flatMap((t) => [t.start, t.dueDate].filter(isIso).map(ym))
      .concat([ym(data.updated)], todayYm ? [todayYm] : []);
  ...
  // ご判断カード・工程表から: 一覧へ移動して該当施策を展開する（対象月の外なら全期間に切り替える）
  const jumpTo = useCallback(
    (id: string) => {
      ...
      if (changed) {
        const qs = p.toString();
        router.replace(`${pathname}${qs ? `?${qs}` : ""}#task-${id}`, { scroll: false });
      }
      // 描画後にスクロール（URL変更→再描画を待つ）
      setTimeout(() => document.getElementById(`task-${id}`)?.scrollIntoView({ behavior: "smooth", block: "center" }), 60);
    },
    [data.tasks, effectiveMonth, todayYm, router, pathname],
  );
  const allShownExpanded = filtered.length > 0 && filtered.every((t) => expanded.has(t.id));
  const toggleAll = () => setExpanded(allShownExpanded ? new Set() : new Set(filtered.map((t) => t.id)));
    ...
    <>
                  ...
                  return (
                    <li key={t.id}>
                      ...
                      >
                        ...
                        <span className="mt-auto text-xs text-bronze-deep">詳細サマリーを見る ↓</span>
                      </button>
                    </li>
                  );
                })}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {view === "list" && filtered.length > 0 && (
              <button
                type="button"
                onClick={toggleAll}
                className="rounded-full border border-line bg-white px-3 py-1.5 text-sm text-ink-soft hover:border-bronze md:text-xs"
              >
                {allShownExpanded ? "詳細をすべて閉じる" : "詳細をすべて開く"}
              </button>
            )}
      ...
      <section className="py-8 md:py-10">
        <div className="mx-auto max-w-6xl space-y-10 px-4 sm:px-6 lg:px-8">
          {view === "timeline" ? (
            <Timeline tasks={filtered} months={effectiveMonth === ALL ? months : [effectiveMonth]} todayIso={todayIso} onOpen={jumpTo} />
          ) : groups.length === 0 ? (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-paper text-left text-xs text-ink-soft">
                        <th className="w-[38%] px-3 py-2 font-medium">施策（押すと詳細）</th>
          ...
          <div className="rounded-xl border border-line bg-white px-5 py-4 text-sm leading-relaxed text-ink-soft md:text-xs">
            ...
            <p className="mt-3">
              「期限超過」は状態とは別に、期限を過ぎた予定・進行中の施策に表示し、完了するまで当月に繰り越します。対象月は「期間がその月と重なる施策」を表示し、期限未定の施策は開始月以降ずっと、日付のない施策は進行中・待ち・予定のものだけ含めます。件数は全期間で集計しています。日付の判定は日本時間です。
            </p>
          </div>
        </div>
      </section>
